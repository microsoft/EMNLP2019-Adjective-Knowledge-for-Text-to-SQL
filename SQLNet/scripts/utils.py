...
def epoch_train(model, optimizer, batch_size, sql_data, table_data, schemas, pred_entry):
    ...
    while st < len(sql_data):
        ...
        score = model.forward(perm, st, ed, q_seq, col_seq, col_num, pred_entry, gt_cond=gt_cond_seq, gt_sel=gt_sel_seq, train=True)
...
def epoch_acc(model, batch_size, sql_data, table_data, schemas, pred_entry, error_print=False, train_flag=False,):
    ...
    while st < len(sql_data):
        ...
        score = model.forward(perm, st, ed, q_seq, col_seq, col_num, pred_entry, train=train_flag)
