...
class SQLNet(nn.Module):
    ...
    def forward(self, perm, st, ed, q, col, col_num, pred_entry,
            gt_where = None, gt_cond=None, gt_sel=None, train=True):
        B = len(q)
        ...
        cond_score = self.cond_pred(perm, st, ed, x_emb_var, x_len, col_inp_var, col_len, col_num, col_name_len, gt_cond=gt_cond,
                                    q_seq=q, col_seq=col, emb_layer=self.embed_layer, train=train)
        group_score = self.group_pred(perm, st, ed, x_emb_var, x_len, col_inp_var, col_len, col_num, col_name_len,
                                    q_seq=q, col_seq=col, emb_layer=self.embed_layer, train=train)
        order_score = self.order_pred(perm, st, ed, x_emb_var, x_len, col_inp_var, col_len, col_num, col_name_len,
                                    q_seq=q, col_seq=col, emb_layer=self.embed_layer, train=train)
