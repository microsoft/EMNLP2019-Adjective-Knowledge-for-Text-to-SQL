...
class WordEmbedding(nn.Module):
    def __init__(self, data_root, word_emb, N_word, gpu, SQL_TOK,
        ...
        self.train_feats = json.load(open(os.path.join(data_root, 'feats/train_feats_{}.json'.format(feats_format))))
        self.dev_feats = json.load(open(os.path.join(data_root, 'feats/dev_feats_{}.json'.format(feats_format))))

        self.pos_feats = np.array(json.load(open(os.path.join(data_root, 'feats/pos_feat.json'))), dtype=np.float32)
        self.neg_feats = np.array(json.load(open(os.path.join(data_root, 'feats/neg_feat.json'))), dtype=np.float32)
        self.feats_format = feats_format


    def get_direction_feature_mask(self, max_q_len, idx, gt_col, train=True):
        dirc_feats = np.zeros([max_q_len], dtype=np.float32)

        feat_dict = self.train_feats[idx] if train else self.dev_feats[idx]
        for q_id, feat_mask in enumerate(feat_dict[gt_col]):
    ...
    def get_direction_feature(self, max_q_len, idx, gt_col, train=True):
        ...
        feat_dict = self.train_feats[idx] if train else self.dev_feats[idx]
        if gt_col in feat_dict.keys():
            for q_id, feats in feat_dict[gt_col].iteritems():
