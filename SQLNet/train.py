...
if __name__ == '__main__':
    ...
    for i in range(300):
        ...
        if val_bkd_acc[0] > best_sel_acc:
            ...
            torch.save(model.sel_pred.state_dict(), "{}/sel_models.dump".format(args.saved_models))
        if val_bkd_acc[1] > best_cond_acc:
            best_cond_acc = val_bkd_acc[1]
            print("Saving cond model...")
            torch.save(model.cond_pred.state_dict(), "{}/cond_models.dump".format(args.saved_models))
        if val_bkd_acc[2] > best_group_acc:
            best_group_acc = val_bkd_acc[2]
            print("Saving group model...")
            torch.save(model.group_pred.state_dict(), "{}/group_models.dump".format(args.saved_models))
        if val_bkd_acc[3] > best_order_acc:
            best_order_acc = val_bkd_acc[3]
            print("Saving order model...")
            torch.save(model.order_pred.state_dict(), "{}/order_models.dump".format(args.saved_models))
