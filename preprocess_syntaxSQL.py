...
data_root_path = r'data/{}'.format(data_type)
data_tgt_path = r'syntaxSQL/data/{}'.format(data_type)
generated_data_path = r'syntaxSQL/generated_data/{}'.format(data_type)

train_data_path = os.path.join(data_root_path, 'train.json')
table_data_path = os.path.join(data_root_path, 'tables.json')
...
if train_dev == "dev":
    train_data_path = os.path.join(data_root_path, 'dev.json')


train_tgt_path = os.path.join(data_tgt_path, 'train.json')
if train_dev == "dev":
    train_tgt_path = os.path.join(data_tgt_path, 'dev.json')
table_tgt_path = os.path.join(data_tgt_path, 'tables.json')
if not os.path.exists(data_tgt_path):
    os.makedirs(data_tgt_path)
if not os.path.exists(table_tgt_path):
    shutil.copy(table_data_path, table_tgt_path)
if not os.path.exists(train_tgt_path):
    shutil.copy(train_data_path, train_tgt_path)
...
def parse_data(data):
    ...
    for key in dataset:
        ...
        json.dump(dataset[key], open(os.path.join(generated_data_path, "{}_{}_{}.json".format(history_option,train_dev, key)), "w"), indent=2)
