...
def epoch_train(model, optimizer, batch_size, component,embed_layer,data, table_type):
    ...
    while st < len(data):
        ...
        if component == "multi_sql":
            ...
            dirc_feats = embed_layer.get_direction_feature(q_seq, perm, st, ed, gt_col, component='op', train=True)
            score = model.forward(q_emb_var, q_len, hs_emb_var, hs_len, col_emb_var, col_len, col_name_len, gt_col=gt_col, dirc_feats=dirc_feats)
            ...
            for i in range(st, ed):
                ...
                gt_col[index] = data[perm[i]]["gt_col"]
                index += 1
            dirc_feats = embed_layer.get_direction_feature(q_seq, perm, st, ed, gt_col, component='des_asc', train=True)
            score = model.forward(q_emb_var, q_len, hs_emb_var, hs_len, col_emb_var, col_len, col_name_len, gt_col=gt_col, dirc_feats=dirc_feats)
    ...
    while st < len(data):
        ...
        if component == "multi_sql":
            ...
            for i in range(st,ed):
                ...
                gt_col[index] = data[perm[i]]["gt_col"]
                index += 1
            dirc_feats = embed_layer.get_direction_feature(q_seq, perm, st, ed, gt_col, component='op', train=False)
            score = model.forward(q_emb_var, q_len, hs_emb_var, hs_len, col_emb_var, col_len, col_name_len, gt_col=gt_col, dirc_feats=dirc_feats)
            ...
            for i in range(st, ed):
                ...
                gt_col[index] = data[perm[i]]["gt_col"]
                index += 1
            dirc_feats = embed_layer.get_direction_feature(q_seq, perm, st, ed, gt_col, component='des_asc', train=False)
            score = model.forward(q_emb_var, q_len, hs_emb_var, hs_len, col_emb_var, col_len, col_name_len, gt_col=gt_col, dirc_feats=dirc_feats)
...
## used in test.py
def test_acc(model, batch_size, data,output_path, table_path):
    table_dict = get_table_dict(os.path.join(table_path, "tables.json"))
    ...
    for idx, item in enumerate(data[:]):
        ...
        sql = model.forward([item["question_toks"]]*batch_size,[],table_dict[db_id], idx)
        if sql is not None:
            print(sql)
