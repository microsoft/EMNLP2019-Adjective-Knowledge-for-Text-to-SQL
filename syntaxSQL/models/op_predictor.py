...
class OpPredictor(nn.Module):
    def __init__(self, N_word, N_h, N_depth, gpu, use_hs, feats_format=""):
        ...
        self.op_num_out_dirc = nn.Linear(50, 50)
        self.op_num_out_dirc_out = nn.Sequential(nn.Tanh(), nn.Linear(50, 2)) #for 4 desc/asc limit/none combinations
        ...
        self.op_out_dirc = nn.Linear(50, 50)
        self.op_out_dirc_out = nn.Sequential(nn.Tanh(), nn.Linear(50, 11))
    ...
    def forward(self, q_emb_var, q_len, hs_emb_var, hs_len, col_emb_var, col_len, col_name_len, gt_col, dirc_feats):
        max_q_len = max(q_len)
        ...
        if self.feats_format == 'direct':
            ...
            dirc_vec = torch.zeros([len(masks), 50])
            zero_feats = torch.zeros([50])
            if self.gpu:
                dirc_vec = dirc_vec.cuda()
                zero_feats = zero_feats.cuda()
            for b, mask in enumerate(masks):
                mask_i = mask.cpu().data.numpy()[0]
                if mask_i > 0:
                    dirc_vec[b] = dirc_feats[1]
                elif mask_i < 0:
                    dirc_vec[b] = dirc_feats[2]
                else:
                    dirc_vec[b] = zero_feats
            dirc_vec = Variable(dirc_vec, requires_grad=False)
        else:
            # [B, max_len, len(feats)]
            dirc_vec = att_prob_qc.unsqueeze(2) * dirc_feats
            dirc_vec = dirc_vec.sum(1)
        op_score = self.op_out(self.op_out_q(q_weighted) + int(self.use_hs)* self.op_out_hs(hs_weighted) + self.op_out_c(col_emb)) + self.op_out_dirc_out(self.op_out_dirc(dirc_vec))
