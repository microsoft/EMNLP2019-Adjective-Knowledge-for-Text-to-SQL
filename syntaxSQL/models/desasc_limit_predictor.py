...
class DesAscLimitPredictor(nn.Module):
    def __init__(self, N_word, N_h, N_depth, gpu, use_hs, feats_format=""):
        ...
        self.dat_out_dirc_out = nn.Sequential(nn.Tanh(), nn.Linear(50, 4)) #for 4 desc/asc limit/none combinations
    ...
    def forward(self, q_emb_var, q_len, hs_emb_var, hs_len, col_emb_var, col_len, col_name_len, gt_col, dirc_feats):
        ...
        dat_score = self.dat_out(self.dat_out_q(q_weighted) + int(self.use_hs)* self.dat_out_hs(hs_weighted) + self.dat_out_c(col_emb)) + self.dat_out_dirc_out(self.dat_out_dirc(dirc_vec))
