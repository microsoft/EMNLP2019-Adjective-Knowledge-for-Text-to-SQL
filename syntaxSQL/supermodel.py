...
def to_batch_tables_new(tables, B, table_type):
    # col_lens = []
    col_seq = []
    ts = [tables["table_names"],tables["column_names"],tables["column_types"]]
    tname_toks = [x.split(" ") for x in ts[0]]
    ...
    cols = [x.split(" ") for xid, x in ts[1]]
    tab_seq = [xid for xid, x in ts[1]]
    cols_add, cols_add_notype, cols_add_notname = [], [], []
    for tid, col, ct in zip(tab_seq, cols, col_type):
        col_one = [ct]
        col_one_notype = []
        if tid == -1:
        ...
        for t in tabn:
            if t not in col:
                col_one.append(t)
                col_one_notype.append(t)

        col_one.extend(col)
        col_one_notype.extend(col)

        cols_add.append(col_one)
        cols_add_notype.append(col_one_notype)
        cols_add_notname.append(col)

    col_seq = [cols_add] * B
    col_seq_notype = [cols_add_notype] * B
    col_seq_notname = [cols_add_notname] * B

    return col_seq, col_seq_notype, col_seq_notname

class SuperModel(nn.Module):
    def __init__(self, data_root, word_emb, N_word, N_h=300, N_depth=2, gpu=True, trainable_emb=False, table_type="std", use_hs=True, feats_format=""):
        ...
        self.op = OpPredictor(N_word=N_word,N_h=N_h,N_depth=N_depth,gpu=gpu, use_hs=use_hs, feats_format=feats_format)
        self.op.eval()
        ...
        self.des_asc = DesAscLimitPredictor(N_word=N_word,N_h=N_h,N_depth=N_depth,gpu=gpu, use_hs=use_hs, feats_format=feats_format)
        self.des_asc.eval()
    ...
    def forward(self,q_seq,history,tables, idx):
        # if self.part:
        #     return self.part_forward(q_seq,history,tables)
        # else:
        return self.full_forward(q_seq, history, tables, idx)

    def full_forward(self, q_seq, history, tables, idx):
        B = len(q_seq)
        # print("q_seq:{}".format(q_seq))
        # print("Batch size:{}".format(B))
        q_emb_var, q_len = self.embed_layer.gen_x_q_batch(q_seq)
        col_seq, col_seq_notype, col_seq_notname = to_batch_tables_new(tables, B, self.table_type)
        ...
        while not stack.isEmpty():
            ...
            #     score = self.having.forward(q_emb_var,q_len,hs_emb_var,hs_len,col_emb_var,col_len,)
                ...
                dirc_feats = self.embed_layer.get_direction_feature_pred(B, q_seq, idx, np.full(B, vet[2], dtype=np.int64))
                score = self.op.forward(q_emb_var, q_len, hs_emb_var, hs_len, col_emb_var, col_len, col_name_len, np.full(B, vet[2],dtype=np.int64), dirc_feats)
                ...
                current_sql[kw].append(index_to_column_name(vet[1], tables))
                current_sql[kw].append(vet[2])
                dirc_feats = self.embed_layer.get_direction_feature_pred(B, q_seq, idx, np.full(B, vet[1], dtype=np.int64))
                score = self.des_asc.forward(q_emb_var, q_len, hs_emb_var, hs_len, col_emb_var, col_len, col_name_len, np.full(B, vet[1],dtype=np.int64), dirc_feats)
                label = np.argmax(score[0].data.cpu().numpy())
